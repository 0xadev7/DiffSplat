...
def startup_event() -> None:
    ...
    logger.info(f"Server up. Port={CFG.port}, GPU={CFG.gpu_id}, Validator={STATE.validator_url}")
...
async def generate(
    ...
    try:
        ...
        elapsed = time() - t0
        logger.info(f"[/generate] score={gen_score:.3f}, attempts={attempts}, total={elapsed:.2f}s")
...
async def generate_video(
    ...
    assert STATE is not None
    t0 = time()
    mp4_buf, gen_score, attempts = await STATE.generate_orbit_mp4_validated(prompt.strip(), res=video_res)
    logger.info(f"[/generate_video] score={gen_score:.3f}, attempts={attempts}, total={time()-t0:.2f}s")
